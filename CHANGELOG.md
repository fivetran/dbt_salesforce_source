[PR #58](https://github.com/fivetran/dbt_salesforce_source/pull/58) includes the following updates:
...
# dbt_salesforce_source v1.2.0

[PR #57](https://github.com/fivetran/dbt_salesforce_source/pull/57) includes the following updates:
...
Found `freshness` as a top-level property of `salesforce` in file `models/salesforce/src_salesforce.yml`. The `freshness` top-level property should be moved
...
If you are using dbt Core < 1.9.6 and want to continue running Salesforce freshness tests, please elect **one** of the following options:
  ...
  2. Do not upgrade your installed version of the `salesforce_source` package. Pin your dependency on v1.1.1 in your `packages.yml` file.
